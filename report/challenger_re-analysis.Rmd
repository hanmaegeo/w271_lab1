...
list.of.packages <- c("tidyverse", "sandwich", "lmtest", "car", "MASS")
...
```{r}
# Create a new Temperature - Mean(Temperature) Column
df$t_minus_t_bar <- df$Temp - mean(df$Temp)
df$O_ring_order <- factor(df$O.ring)

model <- polr(formula = O_ring_order ~ Temp + Pressure, data = df, method = 'logistic')

# glm(formula = O.ring ~ t_minus_t_bar + I(t_minus_t_bar^2), data = df, family = 'poisson')
